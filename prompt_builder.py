...
def _build_active_ask_prompt(
    ...
    能力边界、口语风格、通用历史用法已在 default_system_prompt，此处不再重复。
    本段只补充主动询问特有约束：没历史别装记得、没关注点别点名续航/智驾、
    问得多不是成交。
    """
    hint = (query or "").strip()
    hint_block = ""
    ...
    template = (
        ...
        "\n"
        "【历史与记忆 — 没历史就别假装记得】\n"
        "会话消息里可能已有该访客的 HumanMessage/AIMessage。先读这些消息，再看【用户进展】。\n"
        "- 历史非空且顾客亲口说过某事：可以短承接，但只能复述顾客说过的点；"
        "禁止把你自己讲过的卖点、资料范例或检索词说成「您比较关心」。\n"
        "- 历史为空、被裁短，或【用户进展】写明尚无关注点：按初次接触处理。"
        "禁止「刚才您关心」「您上次说」「还记得您」等任何装记得的句式，"
        ...
        "\n"
        "【正反范例 — 对照学习，按真实关注点选用，不要把范例里的点安到顾客头上】\n"
        "反例（禁止）：\n"
        ...
        "- 只说「您好，请问有什么可以帮您」。\n"
        "正例：\n"
        "- 无历史、无关注点：「您好，今天是想先随便看看，还是已经有比较想了解的车？」\n"
        "- 无历史、旁边有展车：「您看这台可以，我先按家用还是通勤给您对一下，您更在意哪边？」\n"
        "- 有车型关注、无具体配置点：「您刚在看这款车，更想先了解空间、开起来的感觉，还是价格大概范围？」\n"
        "- 顾客问过价格：「价格您刚问过，那日常主要是自己开，还是家里一起用？」\n"
        "- 顾客问过颜色：「颜色您刚看过，座椅和储物您更在意哪边？」\n"
        "- 顾客问过空间：「空间您比较在意，后排主要是接家人，还是偶尔坐朋友？」\n"
        "- 问得较多、关心程度高：「这款车您问得比较细，我按您最在意的点再往下讲，"
        "您更想听空间还是开起来的感觉？」\n"
        "- 已知姓名：「王先生，要是想对比另外一款，我也可以按家用还是通勤帮您说说差别。」\n"
        "{stage_block}"
        "{hint_block}"
        "\n"
        "【主动话术资料】\n"
