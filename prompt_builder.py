...
def _build_active_ask_progress_lines(
    ...
    """
    把按 uuid 读取的导购进度、车型关注、顾客原话档案写成主动招呼约束。

    原话档案是滚动的顾客原句，不是「自己开/单身」这类封闭标签。
    「尚未记录关注点」只表示没问过具体配置，不表示可以重问已经回答过的内容。
    """
    ...
    interest_line = (
        f"已关注（顾客亲口问过车型/配置）：{interest}。\n"
        "这只说明对方更关心这些点，不是成交、留资或锁单信号；"
        "继续围绕已问过的点补一个还没问清的问题，不要跳到下订。\n"
        if interest
        else (
            "尚未记录顾客亲口提出的车型或配置关注点。"
            "这不等于没有对话：【顾客亲口说过】里已有内容就必须承接，禁止当第一次见面。\n"
            "禁止说「刚才您比较关心××」，也不许点名续航、智驾、价格等顾客没提过的具体点。\n"
        )
    )
    utterances = [
        str(x).strip()
        for x in (data.get("customer_utterances") or [])
        if str(x).strip()
    ]
    answered_asks = [
        item
        for item in (data.get("answered_asks") or [])
        if isinstance(item, dict) and str(item.get("ask") or "").strip()
    ]
    last_ask = str(data.get("last_active_ask_text") or "").strip()

    memory_block = ""
    if utterances:
        memory_block += "【顾客亲口说过 — 原话档案，禁止再问已经回答过的内容】\n"
        for line in utterances[-6:]:
            memory_block += f"- {line}\n"
        memory_block += (
            "下一句必须短承接这些原话，禁止把话术资料里的问法再问一遍。"
            "无论主题是用途、预算、车型还是家庭情况，已经说过的都不要再问。\n"
        )
    if answered_asks:
        memory_block += "【已经问过并得到回答】\n"
        for item in answered_asks[-4:]:
            ask = str(item.get("ask") or "").strip()
            reply = str(item.get("reply") or "").strip()
            memory_block += f"- 问过「{ask}」→ 顾客说「{reply}」\n"
        memory_block += "这些问句禁止换皮再问。话术资料若仍是同类问法，必须丢弃。\n"
    if last_ask:
        memory_block += (
            f"【上一句主动问过】\n{last_ask}\n"
            "禁止复述、禁止同义改写后再问同一件事。\n"
    ...
    logger.info(
        "主动招呼进展: new=%s done=%s current=%s interest=%s utterances=%s last_ask=%s",
        is_new,
        done_text,
        current_text,
        interest or "(无)",
        utterances[-3:] if utterances else "(无)",
        (last_ask[:24] + "…") if len(last_ask) > 24 else (last_ask or "(无)"),
    )
    return (
        ...
        f"{memory_block}"
        f"{visit_line}"
        "选择下一句时以历史对话为准：已完成阶段不要重复盘问；"
        "历史或档案里已经出现过的问句/答句不要再问；"
        "只围绕「当前应推进」问一个历史里还没有的信息。\n"
        "若【已关注】里确有顾客问过的点，用口语轻轻接住，不要说成系统记录或数据库。\n"
        "问得多只表示更关心，禁止据此推进成交确认、留电话或锁单。\n"
    )
...
def _build_active_ask_prompt(
    ...
    选问优先级（必须写进 prompt，避免模型照抄检索话术）：
    1. 当前会话里的历史 Human/AI 消息是第一准绳；
    2. 访客原话档案补上被 max_tokens 裁掉的内容；
    3. 话术资料只提供语气，不能覆盖已经问过或答过的内容。
    """
    hint = (query or "").strip()
    ...
    template = (
        ...
        "你的角色是展厅智能导购助手小特。能力边界与口语风格已在系统提示中，这里不再重复。\n"
        "本段覆盖系统提示里「表现出记得对方」的要求：没有顾客亲口内容时，禁止装记得。\n"
        "\n"
        "【选问准绳 — 历史对话优先，禁止重复提问】\n"
        "选下一句时必须按这个顺序，后者不能覆盖前者：\n"
        "1. 当前会话里已经出现的 HumanMessage / AIMessage（顾客说过的、你问过的）是第一准绳。\n"
        "2. 【顾客亲口说过】【已经问过并得到回答】【上一句主动问过】是档案备份："
        "会话历史若被裁短，档案里的原话仍当作已经发生。\n"
        "3. 【用户进展】只决定推进哪个环节，不授权重问已经答过的内容。\n"
        "4. 【主动话术资料】只是语气参考。资料里的问句只要和历史/档案冲突，必须丢掉，禁止照抄。\n"
        "怎么选合适的问题：\n"
        "- 先在历史和档案里划掉已经问过、已经回答过、意思相同或换皮同问的内容。\n"
        "- 再围绕【当前导购阶段目标】，只问一个历史和档案里都还没有的信息。\n"
        "- 有原话就先用几个字短承接，再问那个还缺的点；没有原话才允许轻轻探问来意。\n"
        "- 没有车型关注点 ≠ 没有对话。禁止把「尚无车型关注」理解成可以重问已经回答过的内容。\n"
        "- 历史和档案都为空：按初次接触处理。禁止「刚才您关心」「您上次说」「还记得您」，"
        "也不许点名续航、智驾、价格、颜色等顾客没提过的具体点。\n"
        "- 不要把进展来源说成数据库、记录或系统信息；"
        "禁止把你自己讲过的卖点、资料范例或检索词说成「您比较关心」。\n"
        "\n"
        "{identity_line}\n"
        "{progress_line}\n"
        "{location_line}\n"
        ...
        "2. 以历史对话记录为准选问：先读会话消息和原话档案，划掉已问过/已答过的，"
        "再选一个当前阶段最缺、且历史里没有的问题。"
        "话术资料与历史冲突时，以历史为准，资料问句作废。\n"
        "3. 禁止泛泛问候（如只说「您好请问有什么可以帮您」）。"
        "若确无历史和关注点，才允许一句简短欢迎并轻轻探问来意。\n"
        "4. 可参考【主动话术资料】的风格，但必须改写成自然口语，且不得承诺你做不到的线下动作。\n"
        "5. 禁止在回复中出现资料分类名、文件夹名、tag 名（如 active_ask、ls6 等）"
        "及「根据资料」「检索」等系统用语。\n"
        "6. 若【访客身份】已知姓名，可适当使用；未知则不要臆造称呼。\n"
        "7. 不要编造未经核实的价格、配置或政策数据。\n"
        "8. 问得多只表示更关心该车/该点，禁止据此问下订、留电话、锁单或成交确认。\n"
        "9. 控制在 80 字以内，亲切自然，不压迫，不过度推销。\n"
        "\n"
        "【正反范例 — 学规则，不要把范例里的具体问句安到顾客头上】\n"
        "反例（禁止）：\n"
        "- 顾客没提过任何配置，却点名一个具体卖点，说成「刚才您比较关心××」。\n"
        "- 没有对话历史，却说「还记得您上次来」「您刚才说想买」。\n"
        "- 【顾客亲口说过】或【已经问过并得到回答】里已有的内容，又用话术资料里意思相同的问法再问一遍。\n"
        "- 顾客只是多问了几句这款车，却说「要不要现在定下来」「留个电话方便锁单」。\n"
        "- 只说「您好，请问有什么可以帮您」。\n"
        "正例：\n"
        "- 无原话、无关注点：「您好，今天是想先随便看看，还是已经有比较想了解的车？」\n"
        "- 无原话、旁边有展车：「您看这台可以，想先听开起来的感觉，还是先看空间？」\n"
        "- 有车型关注、无具体配置点：「您刚在看这款车，更想先了解空间、开起来的感觉，还是价格大概范围？」\n"
        "- 档案里已有顾客原话：先用几个字短承接原话，再问一个档案里还没有的点；"
        "禁止把话术资料里已经回答过的问句再问一遍。\n"
        "- 顾客问过某款车的某个点：围绕那个点往下补一个还没问清的信息，不要换一个已问过的点。\n"
        "- 问得较多、关心程度高：「这款车您问得比较细，我按您最在意的点再往下讲，"
        "您更想听空间还是开起来的感觉？」\n"
        "- 已知姓名：可称呼一次，然后问当前阶段最缺、且档案里还没有的一个信息。\n"
        "{stage_block}"
        "{hint_block}"
        "\n"
        "【主动话术资料】\n"
